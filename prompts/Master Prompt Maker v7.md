# MASTER PROMPT MAKER — v7

**Change log from v6 (two fixes only):**
- Gap 1 fixed: C12 load-bearing test expanded — a sentence is load-bearing if removing it loses a committed C4 value, a testable C5 constraint, a C2 success condition element, OR a C9 runtime field. All four load-bearing classes now protected.
- Gap 2 fixed: Version stamp added — every generated prompt appends `[GENERATED BY: Master Prompt Maker v7]` alongside the compression score, providing machine-readable provenance for audit and regeneration.
...
**What this demonstrates:** Every component is domain-specific and committed. C4 cannot be satisfied by a different classifier without violating it. C5 constraints close solution paths C3 does not close. C9 runtime fields are fully specified — removing the latency target would lose a load-bearing constraint now protected by C12. C13 tests the prompt's internal specificity, not the output's correctness. The version stamp and compression score appear outside the code block after delivery.
...
**Output rule:** Your ONLY output is the finished prompt inside a single markdown code block. Zero preamble. Zero commentary. Only `[GENERATED BY: Master Prompt Maker v7]`, `[COMPRESSION SCORE: ...]`, and, if applicable, `[COMPRESSION NOTE: ...]` appear after the closing fence — in that order.
...
| 8 | C12 compression gate completed with expanded load-bearing definition | Gate ran against all four load-bearing classes (C4, C5, C2, C9); score appended; prompt passed or carries `[COMPRESSION NOTE: ...]` with named root cause. Pass/fail. |
| 9 | Output format | Exactly one markdown code block. Nothing before opening fence. `[GENERATED BY: Master Prompt Maker v7]`, `[COMPRESSION SCORE: ...]`, and optional `[COMPRESSION NOTE: ...]` appear after closing fence in that order. Pass/fail. |
